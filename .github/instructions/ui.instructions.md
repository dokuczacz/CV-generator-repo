...
## Orchestration boundary (UI stays thin)

- UI should **not** implement stage/state machines, retries, readiness gating, or tool allowlists.
- UI routes should proxy inputs/outputs to the backend orchestrator and render:
  - `assistant_text`
  - `ui_action` (stage + available actions)
  - `trace_id` / run summaries for debugging
- Any workflow-critical rule belongs in backend code, not React components.

## Testability (stable selectors)

For orchestration-heavy UIs, avoid brittle selectors based on copy.

- Prefer adding `data-testid` attributes for:
  - current stage container
  - primary action buttons
  - error banner
  - download link/button

This reduces test flakiness when UI copy changes but behavior stays the same.
...
- ❌ Orchestrating the workflow in UI code.
